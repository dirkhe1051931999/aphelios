...
@Component({
  name: 'myEditorPostEmbedVideoComponent',
  components: {
    VueTagsInput,
    PostAlbumComponent,
    PostVideoComponent,
...
export default class myEditorPostEmbedVideoComponent extends commonPost {
  declare $refs: {
  ...
  watchBlogEditorPostVisiableVideoEmbed(val: boolean) {
    if (val) {
      if (BlogPostModule.postAddOrUpdateVideoEmbed === 'update') onEditorVisiableShow(this, BlogPostModule.postDetailVideoEmbed.row, '5');
  ...
  public async handleConfirmAddOrUpdate() {
    ...
    if (this.validateParams(this.dialogEditorParams.params, validations)) {
      ...
      if (result) {
        try {
          const params: any = onBeforeEditorDone(this, '5');
          this.$q.loading.show();
          if (BlogPostModule.postAddOrUpdateVideoEmbed === 'add') {
            await BlogPostModule.addVideoEmbedPost(params);
            BlogPostModule.SET_ADD_POST_SUCCESS_FLAG_VIDEO_EMBED(true);
          } else {
            params.id = this.dialogEditorParams.params.id;
            await BlogPostModule.updateVideoEmbedPost(params);
            BlogPostModule.SET_UPDATE_POST_SUCCESS_FLAG_VIDEO_EMBED(true);
          }
          this.hide();
          this.$q.loading.hide();
