...
// 新增或修改文章时添加新标签（如果标签已存在，则返回标签id）
export const addNewTagWhenPost = async (ctx) => {
...
// 添加文章标签
export const addNewTag = async (ctx) => {
  let name = ctx.params.name || 0;
  try {
    let existName = await ctx.execSql(`SELECT * FROM tag WHERE name = ?`, name);
    if (existName.length > 0) {
...
// 更新文章标签
export const updateTag = async (ctx) => {
  let id = ctx.params.id || 0,
    name = ctx.query.name || '';
  try {
...
// 搜索标签
export const searchTagByName = async (ctx) => {
