...
export const login = async (ctx: Context): Promise<void> => {
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      const session_code = await ctx.redisDB.get(`${userName}-${email}-code-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
      if (!session_code || (session_code && String(session_code) !== String(code))) {
        ...
        if (hashedPassword === hashPassword) {
          ...
          ctx.redisDB.set(`${email}-${userName}-${userId}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`, userToken, 1000 * 60 * 60 * 24);
    ...
    if (results.length > 0) {
      ...
      if (hashedPassword === hashOldPassword) {
        ...
        if (ctx.request.headers.authorization) {
          ...
          if (ctx.redisDB.get(`${results[0].email}-${userName}-${results[0].id}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`)) {
            ctx.redisDB.destroy(`${results[0].email}-${userName}-${results[0].id}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
          }
        }
...
export const changePasswordWithOutOld = async (ctx: Context): Promise<void> => {
  ...
  try {
    let changePasswordToken = await ctx.redisDB.get(`${token}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
    if (!changePasswordToken) {
      ctx.error(ctx, 112);
    } else {
      const { username, email } = await ctx.redisDB.get(`${token}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
      ...
      if (results.length > 0) {
        ...
        // 删除重置密码链接
        await ctx.redisDB.destroy(`${token}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
        ctx.success(ctx, null);
      } else {
        ctx.error(ctx, 102);
      }
...
export const signOut = async (ctx: Context): Promise<void> => {
  ...
  await ctx.redisDB.destroy(`${email}-${userName}-${userId}-${ctx.request.headers['client-id']}-${ctx.request.headers['client-type']}`);
  ctx.success(ctx, null);
};
