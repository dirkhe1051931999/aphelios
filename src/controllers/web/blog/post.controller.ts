...
export const getPostById = async (ctx) => {
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      let prevNextResults: [{ sort: number }] = await ctx.execSql(prevNextSql);
      let prev = {},
        next = {};
...
export const getArchive = async (ctx) => {
  ...
  try {
    ...
    let tagResults = await ctx.execSql(tagSql);
    let timeResults: [
      { yearMonth: string; id: string; title: string; createTime: string }
    ] = await ctx.execSql(timeSql);
...
// 通过类别搜索
export const getPostsByCategoryId = async (ctx) => {
  let id = ctx.params.id || 0,
    ...
    sql = ` SELECT post.id, post.title, post.content, 
              ...
              ORDER BY post.createTime DESC LIMIT ${pageIndex}, ${pageNum}`;
  try {
    let category = await ctx.execSql(
...
// 通过标签搜索
export const getPostsByTagId = async (ctx) => {
  let id = ctx.params.id || 0,
    page = ctx.query.page || 1,
    pageNum = ctx.query.pageNum || 10,
    pageIndex = (page - 1) * pageNum < 0 ? 0 : (page - 1) * pageNum,
...
// 通过关键字搜索
export const getPostsByKeyword = async (ctx) => {
...
// 获取个人作品
export const getSelfItems = async (ctx) => {
  try {
    let results = await ctx.execSql(
      `SELECT * FROM laboratory ORDER BY createTime DESC`
    );
    ctx.body = {
      success: 1,
      message: "",
