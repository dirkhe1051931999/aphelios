...
export const addComment = async (ctx) => {
  ...
  try {
    ...
    let payload = await verify(
      ctx.request.body.token.split(" ")[1],
      CONFIG.tokenSecret
    );
  ...
  try {
    let maxData = await ctx.execSql(
      "SELECT IFNULL(max(number), 0) AS maxNumber FROM comments WHERE postId = ?",
      commentData.postId
    );
...
export const getCommentsByPostId = async (ctx) => {
  ...
  let sql = `SELECT comments.*, user.avatar as fromAvatar, user.userName as fromUserName
        FROM comments
